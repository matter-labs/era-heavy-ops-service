...
pub fn compute_lookup_s_values<S: SynthesisMode, MC: ManagerConfigs>(
    ...
    let (max_size_of_table, max_size_of_indexes, all_cols_size) =
        get_max_table_and_max_witness_size(assembly)?;

    let mut buffers = create_buffers_for_s_col_computation(
        manager,
        max_size_of_table,
        max_size_of_indexes,
        all_cols_size,
...
fn get_max_table_and_max_witness_size<S: SynthesisMode>(
    assembly: &DefaultAssembly<S>,
    ...
    for table_name in assembly.known_table_names.iter() {
        let num_rows_of_original_table = assembly
            ...
            .get(table_name)
            .ok_or(GpuError::AssemblyError(format!(
        ...
        let num_rows_of_witnesses = assembly
            ...
            .get(table_name)
            .ok_or(GpuError::AssemblyError(format!(
                "couldn't file row indexes for the table {}",
                table_name
            )))?
            .len();
...
fn create_buffers_for_s_col_computation<MC: ManagerConfigs>(
    ...
    manager.set_values_with_range(PolyId::S, PolyForm::Values, Fr::zero(), 0..s_start)?;
    ...
    for ctx_id in 0..MC::NUM_GPUS {
        let table_buff = DeviceBuf::async_alloc_in_h2d(&manager.ctx[ctx_id], table_length)?;
        let index_buff = DeviceBuf::async_alloc_in_h2d(&manager.ctx[ctx_id], indexes_length)?;
        let sorted_idx_buff = DeviceBuf::async_alloc_in_exec(&manager.ctx[ctx_id], indexes_length)?;
...
fn get_s_part_from_table<S: SynthesisMode, MC: ManagerConfigs>(
    manager: &mut DeviceMemoryManager<Fr, MC>,
    assembly: &DefaultAssembly<S>,
    table_name: &str,
    ...
    let num_current_table_rows = assembly
        .individual_table_canonical_sorted_entries
        .get(table_name)
        .unwrap()
        .len();
    ...
    upload_indexes(manager, assembly, table_name, &mut buffers.1, ctx_id)?;
...
fn upload_columns_lc<S: SynthesisMode, MC: ManagerConfigs>(
    manager: &mut DeviceMemoryManager<Fr, MC>,
    assembly: &DefaultAssembly<S>,
    table_name: &str,
...
fn upload_indexes<S: SynthesisMode, MC: ManagerConfigs>(
    manager: &mut DeviceMemoryManager<Fr, MC>,
    assembly: &DefaultAssembly<S>,
    table_name: &str,
...
fn sort_indexes<MC: ManagerConfigs>(
    ...
    unsafe {
        ...
        let mem_pool = manager.ctx[ctx_id]
            .mem_pool
            .expect("mem pool should be allocated");
